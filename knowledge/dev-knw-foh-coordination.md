...
It supervises manager processes, reaps dead managers and orphaned processes, and drives the sweeps that wake awaiting sessions, fire cron schedules, keep stations served, and run the un-returned-turn reaper. It does not respawn per-session waiter processes — there are none. See [[dev-knw-foh-orchestrator]].
