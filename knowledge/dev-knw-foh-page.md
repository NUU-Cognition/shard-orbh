...
- `step`, `skip`, `finish`, and `abandon` fire the workflow chokepoint (`start` does not), which is how cross-session clauses hang off individual steps — see [[dev-knw-foh-machinery]].
