...
- `--into` takes a **profile registry key** — the same `runtime/profile` target `launch`, `request`, and `i` accept (see [[dev-knw-foh-profiles]]). Confirm it with `flint orbh profiles` first; resolution is exact and an unknown name errors.
...
>
> Use `--into` when the **runtime** changes. That is the case a native conversation genuinely cannot survive, and the handoff is how meaning crosses the gap. To change the **account** while preserving the conversation, use `auth migrate` ([[dev-knw-foh-cli]]).
